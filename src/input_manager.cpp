...
{
	// Sets the mouse mouse grabbing mode
...
{
	return Ogre::Vector2((float)mMouseData[0], (float)mMouseData[1]);
}
...
{
	return Ogre::Vector2((float)mMouseData[2], (float)mMouseData[3]);
}



void ssuge::InputManager::setMouseVisible(bool show)
{
			...
			else if (sections[i].mLabel == "gamepad-bindings")
			{
				for (unsigned int j = 0; j < sections[i].mPairs.size(); j++)
				{
					...
					if (key == "A")
						...
						mGamepadAxisMultiplier[std::pair<std::string, unsigned int>(parts[0], 0)] = (float)ssuge::to_double(parts[1]);
					else if (key == "right-analog" && parts.size() > 1 && is_double(parts[1]) && (parts[0] == "horizontal" || parts[0] == "vertical"))
						mGamepadAxisMultiplier[std::pair<std::string, unsigned int>(parts[0], 1)] = (float)ssuge::to_double(parts[1]);
					else if (key == "left-thumb" && parts.size() > 1 && is_double(parts[1]) && parts[0] == "other")
						mGamepadAxisMultiplier[std::pair<std::string, unsigned int>("other", 0)] = (float)ssuge::to_double(parts[1]);
					else if (key == "right-thumb" && parts.size() > 1 && is_double(parts[1]) && parts[0] == "other")
						mGamepadAxisMultiplier[std::pair<std::string, unsigned int>("other", 1)] = (float)ssuge::to_double(parts[1]);
...
void ssuge::InputManager::processMouseMove(const SDL_Event & e)
{
	...
	mMouseData[0] = (float)e.motion.x / w;
	mMouseData[1] = (float)e.motion.y / h;
	mMouseData[2] = (float)e.motion.xrel / w;
	mMouseData[3] = (float)e.motion.yrel / h;
...
void ssuge::InputManager::broadcastTouch(unsigned int touch_num, float x, float y, float xrel, float yrel)
{
	for (unsigned int i = 0; i < mListeners.size(); i++)
